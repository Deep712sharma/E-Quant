...
class skip_LDM_Model():
    ...
    def set_skip_model(self, model: nn.Module):
        for name, module in model.named_modules():
            ...
            if self.model_type in ["imagenet", "stable", "bedroom"]:
            ...
            elif self.model_type in ["celeba", "celeba256"]:
                # Adjusted skip indices for CelebA256 — most LDM celeba256 models have 12 output blocks
                if name == "model.input_blocks.1.0":
                    self.skip = 1
                elif name == "model.output_blocks.11.0":
                    self.skip = 2
                ...
                raise ValueError(f"Undefined model type: {self.model_type}")
